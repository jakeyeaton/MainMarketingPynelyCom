/*
Business Info Label - Label
*/
//Business Info Label - Location Label
new Vue({
	el: '#footer-location-label',
	data: {
		title: 'Feito em:'
	}
});
//Business Info Label - Location City
new Vue({
	el: '#footer-location-city',
	data: {
		title: 'Seattle, Wa'
	}
});
//Business Info Label - Short Discription
new Vue({
	el: "#footer-short-description",
	data: {
		title: 		"Criamos marketing digital, design e serviços de desenvolvimento que são simples."

	}
});
//Business Info Label - Call Us Label
new Vue({
	el: '#footer-call-us-label',
	data: {
		title: "Ligue-nos"
	}
});
//Business Info Label - Call Us Hours
new Vue({
	el: "#footer-call-us-hours",
	data: {
		title: "2a. a 6a. 8 am-5pm (PST) "
	}
});
//Business Info Label - Call Us Number
new Vue({
	el: "#footer-call-us-number",
	data: {
		title: "+1 888 629-0687"
	}
});
//Business Info Label - Email Label
new Vue({
	el: "#footer-email-label",
	data: {
		title: "O email:"
	}
});
//Business Info Label - Email Address
new Vue({
	el: "#footer-email-address",
	data: {
		title: "hello@pynely.com"
		}
});
//Business Info Label - Chat Label
new Vue({
	el: "#footer-chat-label",
	data: {
		title: "Converse conosco:"
	}
});
//Business Info Label - Chat Trigger
new Vue({
	el: "#footer-chat-trigger",
	data: {
		title: "Bate-papo ao vivo"
	}
});

//Services - Main Label
new Vue({
	el: "#footer-services-label",
	data: {
		title: "Serviços"
	}
});
//Services - Development Services Label
new Vue({
	el: "#footer-lable-services-development",
	data: {
		title: "Serviços de desenvolvimento"
	}
});
//Services - Mobile App Development Footer Link
new Vue({
	el: "#footer-services-mobile-app-dev",
	data: {
		title: 'Serviços de aplicativos para dispositivos móveis',
		url: 'https://pynely.com/br/services/development/mobile-app.html'
	}
});
//Services - WordPress Development Footer Link
new Vue({
	el: "#footer-services-wordpress-dev",
	data: {
		title: 'Desenvolvimento WordPress',
		url: 'https://pynely.com/br/services/development/wordpress.html'
	}
});
//Services - Design Services Label
new Vue({
	el: '#footer-label-services-design',
	data: {
		title: 'Serviços de Design'
	}
});
//Services - Mobile App UX/UI Footer Link
new Vue({
	el: '#footer-services-mobile-app-uxui',
	data: {
		title: 'Aplicativo móvel UX / UI',
		url: 'https://pynely.com/br/services/design/mobile-app-ux-ui.html'
	}
});
//Services - Website UX/UI Footer Link
new Vue({
	el: '#footer-services-website-uxui',
	data: {
		title: 'Website UX / UI',
		url: 'https://pynely.com/br/services/design/website-ux-ui.html'
	}
});
//Services - Marketing Services Label
new Vue({
	el: "#footer-label-services-marketing",
	data: {
		title: "Serviços de Marketing"
	}
});
//Services - Social Media Marketing Footer Link
new Vue({
	el: '#footer-services-socialMediaMarketing',
	data: {
		title: 'Marketing de mídia social',
		url: 'https://pynely.com/br/services/marketing/social-media-marketing.html'
	}
});
//Services - Pay-Per-Click Ads Footer Link
new Vue({
	el: "#footer-services-ppc",
	data: {
		title: 'Anúncios de Pagamento por clique',
		url: 'https://pynely.com/br/services/marketing/pay-per-click-ads.html'
	}
});


/*
dfdhjfh
*/
//Twitter Feed Label
new Vue({
	el: "#footer-twitter-feed-label",
	data: {
		title: "Twitter Feed"
	}
});
//Instagram Feed Label
new Vue({
	el: "#footer-instagram-feed-label",
	data: {
		title: "Instagram Feed"
	}
});




/*
Bottom Footer Links Section
*/

//Terms of Service - Bottom Footer
new Vue({
	el: '#footer-bottom-terms',
	data: {
		title: 'Termos de serviço',
		url: "https://pynely.com/br/policies/terms.html"
	}
});

//Privacy Policy - Bottom Footer
new Vue({
	el: '#footer-bottom-privacy',
	data: {
		title: 'Privacidade',
		url: "https://pynely.com/br/policies/privacy.html"
	}
});

//Jobs - Bottom Footer
new Vue({
	el: '#footer-bottom-jobs',
	data: {
		title: 'Empregos',
		url: "https://pynely.com/en/jobs/main.html"
	}
});
//Blog - Bottom Footer
new Vue({
	el: '#footer-bottom-blog',
	data: {
		title: 'Blog',
		url: "https://blog.pynely.com"
	}
});

//Company News - Bottom Footer
new Vue({
	el: '#footer-bottom-companyNews',
	data: {
		title: 'Notícias da Empresa',
		url: "https://blog.pynely.com/tagged/company-news"
	}
});

//Partners - Bottom Footer
new Vue({
	el: '#footer-bottom-partners',
	data: {
		title: 'Parceiros',
		url: 'https://www.pynely.com/br/partners/home.html'
	}
});

//About us - Bottom Footer
new Vue({
	el: '#footer-bottom-about',
	data: {
		url: 'https://pynely.com/br/about.html',
		title: 'Sobre nós'
	}
});
//Copyright Year - Bottom Footer
new Vue({
	el: '#footer-copyright-year',
	data: {
		title: "2016"
	}
});







/*
Social Media Icon Section
*/

//Social Label - Bottom Footer
new Vue({
	el: '#footer-social-label',
	data: {
		title: "Dê-nos um seguimento"
	}
})
//Facebook Icon - Bottom Footer
new Vue({
	el: '#footer-facebook-icon',
		data: {
			url: "https://www.facebook.com/pynelyllc"
		}
});
//Twitter Icon - Bottom Footer
new Vue({
	el: '#footer-twitter-icon',
		data: {
			url: "https://twitter.com/pynely"
		}
});
//LinkedIn Icon - Bottom Footer
new Vue({
	el: '#footer-linkedin-icon',
		data: {
			url: "https://www.linkedin.com/company/pynely"
		}
});
//Instagram Icon - Bottom Footer
new Vue({
	el: '#footer-instagram-icon',
		data: {
			url: "https://www.instagram.com/pynelyllc/"
		}
});
//Google Plus Icon - Bottom Footer
new Vue({
	el: '#footer-google-plus-icon',
		data: {
			url: "https://plus.google.com/+Pynelyhq"
		}
});
//Pinterest Icon - Bottom Footer
new Vue({
	el: '#footer-pinterest-icon',
		data: {
			url: "https://www.pinterest.com/pynely/"
		}
});
//Behance Icon - Bottom Footer
new Vue({
	el: '#footer-behance-icon',
		data: {
			url: "https://www.behance.net/pynely"
		}
});
